...
const GeminiChatPage: React.FC<GeminiChatPageProps> = ({ userData }) => {
  ...
  const raw = sessionStorage.getItem(`fitbuddyai_chat_${uid}`);
    ...
    window.addEventListener('fitbuddyai-login', onLogin);
    ...
    const onOpen = async () => {
      try {
        ...
        const mod = await import('../services/cloudBackupService');
        await mod.restoreUserDataFromServer(String(uid));
        const raw = localStorage.getItem(`fitbuddyai_chat_${uid}`);
        isRestoringRef.current = false;
        if (!raw) return;
        const parsed = JSON.parse(raw);
        if (Array.isArray(parsed) && parsed.length) setMessages(parsed);
  ...
  useEffect(() => {
    const onStorage = (e: StorageEvent) => {
      try {
        if (!e.key) return;
        const uid = userData?.id || 'anon';
        const expectedKey = `fitbuddyai_chat_${uid}`;
  ...
  useEffect(() => {
      ...
      try {
        const uid = userData?.id || 'anon';
        try { sessionStorage.removeItem(`fitbuddyai_chat_${uid}`); } catch {}
        try { localStorage.removeItem(`fitbuddyai_chat_${uid}`); } catch {}
        setMessages([{ role: 'assistant', text: ASSISTANT_GREETING }]);
      } catch {}
    };
    window.addEventListener('fitbuddyai-logout', onLogout);
    return () => window.removeEventListener('fitbuddyai-logout', onLogout);
  }, [userData?.id]);
    ...
    const handle = () => {
      ...
      if ((prevId === undefined || prevId === 'anon') && currentId && currentId !== 'anon') {
        try {
          ...
          const anonRaw = sessionStorage.getItem('fitbuddyai_chat_anon');
          if (anonRaw) {
            try {
              sessionStorage.setItem(`fitbuddyai_chat_${currentId}`, anonRaw);
            } catch {}
          }
          // remove anon key
  ...
  try { sessionStorage.setItem('fitbuddyai_chat_anon', JSON.stringify(messages)); } catch {}
  ...
  const truncatedHistory = recent.slice(Math.max(recent.length - MAX_HISTORY_MESSAGES, 0));
  const conversation = truncatedHistory.map(m => `${m.role === 'user' ? 'User' : 'Assistant'}: ${m.text}`).join('\n');
  ...
  const questionnaireRaw = localStorage.getItem('fitbuddyai_questionnaire_progress');
  const userRaw = sessionStorage.getItem('fitbuddyai_user_data');
  const planRaw = localStorage.getItem('fitbuddyai_workout_plan');
